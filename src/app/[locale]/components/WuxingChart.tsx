/**
 * src/app/[locale]/components/WuxingChart.tsx
 * 五行五边形生克图（参考图3）：
...
/** 方位中文 → report.dir 叶子键（复用报告 i18n） */
const DIR_KEY: Record<string, string> = {
  东: 'dong', 南: 'nan', 西: 'xi', 北: 'bei', 中: 'zhong',
  东南: 'dongnan', 西南: 'xinan', 西北: 'xibei', 东北: 'dongbei',
};

export function WuxingChart({ result }: { result: MaybeSimplePaipanResult }) {
  const t = useTranslations();
  const locale = useLocale();
  /** 列表分隔符本地化：中文顿号，英文逗号 */
  const listSep = locale === 'zh' ? '、' : ', ';
  const termT = (term: string) => { const k = termKey(term); return k ? t(k) : term; };
  const score = result.wuxingScore;
  const luckyWx = [...result.xiYong.yong, ...result.xiYong.xi];

  return (
    <div>
      <svg viewBox="-6 0 452 400" className="wuxing-svg" role="img" aria-label={t('wuxing.ariaChartLabel')}>
        <defs>
        ...
        {KE_LINES.map(([a, b], i) => {
          ...
          return (
            <g key={`ke-${i}`}>
              ...
              <text x={mx} y={my + 3} textAnchor="middle" fontSize="11" fill="#b09a80">{t('wuxing.ke')}</text>
            </g>
          );
        })}
          ...
          return (
            <g key={`sheng-${i}`}>
              ...
              <text x={a.label.x} y={a.label.y} textAnchor="middle" fontSize="13"
                    fill="#8a6d4b" fontWeight="bold">{t('wuxing.sheng')}</text>
            </g>
          );
        })}
              ...
              {n.dayMaster ? (
                <g>
                  ...
                  <text x={n.lx} y={n.ly} textAnchor="middle" fontSize="13"
                        fill="#fff" fontWeight="bold">{t('wuxing.riZhu')}</text>
                </g>
              ) : (
          ...
          return (
            <span key={w.wuxing} className={`wx-item ${isDayMaster ? 'wx-daymaster' : ''}`}>
              ...
              <span style={{ color: WX_COLOR[w.wuxing], fontWeight: isDayMaster ? 800 : 700 }}>
                {w.wuxing}{termT(w.state)}
              </span>
              {isDayMaster && <span className="wx-dm-tag">{t('wuxing.riYuan')}</span>}
            </span>
          );
        })}
      </div>
      ...
      <div className="xiyong-divider">
        <span className="dot" />
        {t('wuxing.xiyongOpen')}{result.xiYong.yong.join(listSep)}
        {result.xiYong.xi.length > 0 ? `${listSep}${result.xiYong.xi.join(listSep)}` : ''}{t('wuxing.xiyongClose')}
        <span className="dot" />
      </div>

      <div className="lucky-row">
        <div className="lucky-item">
          <div className="lucky-label">{t('wuxing.luckyColor')}</div>
        ...
        <div className="lucky-item">
          <div className="lucky-label">{t('wuxing.luckyDirection')}</div>
          <div className="lucky-tags">
            {result.xiYong.directions.map(d => (
              <span key={d} className="lucky-tag">{DIR_KEY[d] ? t(`report.dir.${DIR_KEY[d]}`) : d}</span>
            ))}
          </div>
        </div>
        <div className="lucky-item">
          <div className="lucky-label">{t('wuxing.luckyNumber')}</div>
          <div className="lucky-tags">
