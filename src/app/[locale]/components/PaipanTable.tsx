/**
 * src/app/[locale]/components/PaipanTable.tsx
 * 四柱横向表（参考图1.2）：
 * 行=干神/天干/地支/藏干/支神/纳音/空亡/地势/自坐/神煞，列=年/月/日/时柱；
 * 底部两行为天干、地支作用关系（紧凑文字）。
 */
'use client';
import type { MaybeSimplePaipanResult, Pillar, GanZhiRelation } from '@/lib/bazi/paipan';
...
const COLS = [
  { key: 'year', titleKey: 'yearPillar' },
  { key: 'month', titleKey: 'monthPillar' },
  { key: 'day', titleKey: 'dayPillar' },
  { key: 'hour', titleKey: 'hourPillar' },
] as const;
...
function buildPosMap(pillars: MaybeSimplePaipanResult['pillars']) {
 ...
 */
function summarizeDizhi(
  relations: GanZhiRelation[],
  zhiPos: Record<string, number[]>,
  termT: (term: string) => string,
): string[] {
  ...
  for (const r of relations) {
    ...
    let order: number;
    switch (r.type) {
      case '冲': text = `${pair}${termT('冲')}`; order = 2; break;
      case '刑': text = `${pair[0]}${termT('刑')}${pair[1]}`; order = 1; break;
      case '破': text = `${pair}${termT('破')}`; order = 3; break;
      case '害': text = `${pair}${termT('害')}`; order = 4; break;
...
function summarizeTiangan(
  relations: GanZhiRelation[],
  ganPos: Record<string, number[]>,
  termT: (term: string) => string,
): string[] {
  const out: string[] = [];
  ...
  for (const want of ['冲', '克', '合']) {
    for (const r of rels) {
      ...
      if (want === '冲') out.push(`${normPair(TG_CHONG, r.from, r.to)}${termT('冲')}`);
      else if (want === '克') out.push(`${r.from}${termT('克')}${r.to}`);
...
function ShenshaCell({
  ...
  return (
    <div className="ss-stack">
      {names.map(n => <span key={n} className="shensha-tag">{termT(n)}</span>)}
    </div>
  );
}

export function PaipanTable({ result, gender }: { result: MaybeSimplePaipanResult; gender: 'male' | 'female' }) {
  ...
  const tgText = summarizeTiangan(result.relations, ganPos, termT).join(' · ');
  const dzText = summarizeDizhi(result.relations, zhiPos, termT).join(' · ');
      ...
      <table className="paipan-table">
        <thead>
          <tr>
            ...
            {COLS.map(c => <th key={c.key} className={c.key === 'day' ? 'col-day' : ''}>{t('pillar.' + c.titleKey)}</th>)}
          </tr>
        </thead>
        <tbody>
          {/* 干神 */}
          <tr>
            {label(t('pillar.ganShen'))}
            {COLS.map(c => {
              const p = result.pillars[c.key];
              const text = c.key === 'day' ? (gender === 'female' ? '女主' : '男主') : p.shishen;
              const display = c.key === 'day'
                ? (gender === 'female' ? t('pillar.femaleMaster') : t('pillar.maleMaster'))
                : termT(p.shishen);
              return (
                <td key={c.key} className={c.key === 'day' ? 'col-day' : ''}>
                  <span className="gan-shen" style={{ color: ssColor(dayWx, text) }}>{display}</span>
                </td>
              );
            })}
          </tr>
            ...
            {COLS.map(c => {
              ...
              return (
                <td key={c.key} className={c.key === 'day' ? 'col-day' : ''}>
                  <div className="cg-stack">
                    {p.zhishen.map((ss, i) => (
                      <div key={i} style={{ color: ssColor(dayWx, ss) }}>{termT(ss)}</div>
                    ))}
                  </div>
                </td>
              );
          ...
          {/* 地势 */}
          <tr>
            {label(t('pillar.diShi'))}
            {COLS.map(c => {
              const p = result.pillars[c.key];
              return <td key={c.key} className={c.key === 'day' ? 'col-day' : ''}>{termT(p.dishi)}</td>;
            })}
          </tr>
          {/* 自坐 */}
          <tr>
            {label(t('pillar.ziZuo'))}
            {COLS.map(c => {
              const p = result.pillars[c.key];
